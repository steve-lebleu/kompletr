<?php
$request       = $_POST['requestExpression'];   // Request expression
$responseSize  = 5;                             // Number of items to return
$expire        = time() - 3600;                 // Validity cache duration in seconds

strpos($_SERVER['DOCUMENT_ROOT'], 'wamp') !== FALSE ? $filePath = $_SERVER['DOCUMENT_ROOT'] . 'e-lless/www/demos/completer/files/completer.json' : $filePath = $_SERVER['DOCUMENT_ROOT'] . '/demos/completer/files/completer.json';
strpos($_SERVER['DOCUMENT_ROOT'], 'wamp') !== FALSE ? $cfgPath = $_SERVER['DOCUMENT_ROOT'] . 'e-lless/www/demos/completer/files/configuration.ini' : $cfgPath =  $_SERVER['DOCUMENT_ROOT'] . '/demos/completer/files/configuration.ini';
...
if(file_exists($cfgPath)) 
{
    ...
    if(!file_exists($filePath) || filemtime($filePath) > $expire)
    {
        try 
        {
            $pdo = new PDO($cfg["dsn"], $cfg["user"], $cfg["pwd"], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION , PDO::ATTR_PERSISTENT => FALSE));
        } 
