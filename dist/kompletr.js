...
class Broadcaster {
  ...
  trigger(type) {
    ...
    this.subscribers.filter(subscriber => subscriber.type === type).forEach(subscriber => subscriber.handler(detail));
  }
}
...
class Cache {
  ...
  constructor(broadcaster) {
    ...
    let name = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'kompletr.cache';
    if (!window.caches) {
      return false;
    }
   ...
   */
  get(string, done) {
    window.caches.open(this._name).then(cache => {
      cache.match(string).then(async data => {
  ...
  set(_ref) {
    ...
    window.caches.open(this._name).then(cache => {
      ...
      cache.put(`/${string}`, new Response(JSON.stringify(data), {
        headers
      }));
    }).catch(e => {
      this._broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_0__.event.error, e);
...
class DOM {
  ...
  constructor(input, broadcaster) {
    ...
    this.result = this.build('div', [{
      id: this._identifiers.results
    }, {
      class: this._classes.results
    }]);
    this.wrapper = this.build('div', [{
      id: this._identifiers.wrapper
    }, {
      class: this._classes.results
    }]);
  ...
  */
  ...
  buildResults(data, fieldsToDisplay) {
    ...
    this._broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_0__.event.domDone, this.result);
  }
}
...
class Kompletr {
   ...
   */
  showResults = async _ref2 => {
    let {
      from,
      data
    } = _ref2;
    this.props.data = data;
    data = this.props.data.map((record, idx) => ({
      idx,
      data: record
    ...
    try {
      if (this.configuration.cache && (await this.cache.isValid(value))) {
        this.cache.get(value, data => {
          this.broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_1__.event.dataDone, {
            from: _enums_js__WEBPACK_IMPORTED_MODULE_1__.origin.cache,
            data: data
          });
        });
        ...
        this.callbacks.onKeyup(value, data => {
          this.broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_1__.event.dataDone, {
            from: _enums_js__WEBPACK_IMPORTED_MODULE_1__.origin.callback,
            data: data
          });
        });
      } else {
        this.broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_1__.event.dataDone, {
          from: _enums_js__WEBPACK_IMPORTED_MODULE_1__.origin.local,
  ...
  select = (() => {
    ...
    return function () {
      ...
      try {
        _this.dom.input.value = typeof _this.props.data[idx] === 'object' ? _this.props.data[idx][_this.configuration.propToMapAsValue] : _this.props.data[idx];
        _this.callbacks.onSelect && _this.callbacks.onSelect(_this.props.data[idx]);
        _this.broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_1__.event.selectDone);
      } catch (e) {
        _this.broadcaster.trigger(_enums_js__WEBPACK_IMPORTED_MODULE_1__.event.error, e);
      }
    };
  })();
}
...
const kompletr = function (_ref) {
  ...
  try {
    const [broadcaster, configuration, properties] = [new _broadcaster_js__WEBPACK_IMPORTED_MODULE_3__.Broadcaster(), new _configuration_js__WEBPACK_IMPORTED_MODULE_1__.Configuration(options), new _properties_js__WEBPACK_IMPORTED_MODULE_4__.Properties(data)];
    const [dom, cache] = [new _dom_js__WEBPACK_IMPORTED_MODULE_5__.DOM(input, broadcaster, configuration), configuration.cache ? new _cache_js__WEBPACK_IMPORTED_MODULE_2__.Cache(broadcaster, configuration.cache) : null];
    new _kompletr_js__WEBPACK_IMPORTED_MODULE_0__["default"]({
      configuration,
      properties,
      dom,
      cache,
...
/******/ })()
;
//# sourceMappingURL=kompletr.js.map